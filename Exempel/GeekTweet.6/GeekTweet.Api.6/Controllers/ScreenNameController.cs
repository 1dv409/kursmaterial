...
namespace GeekTweet.Api.Controllers
{
    public class ScreenNameController : ApiController
    {
        ...
        public HttpResponseMessage Get([FromUri]string term)
        {
            var response = Request.CreateResponse<IEnumerable<string>>(HttpStatusCode.OK, _service.GetScreenNames(term));
        ...
        {
            var response = Request.CreateResponse<IEnumerable<string>>(HttpStatusCode.OK, _service.GetScreenNames(model.Term));
            response.Headers.Add("Access-Control-Allow-Origin", "*");
            return response;
        }
